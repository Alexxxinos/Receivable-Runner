...
// Match an uploaded filename to an invoice number. Filenames look like
// "<invoice-no> <address>.pdf", so we prefer a match at the start of the name
// and, on ties, the longest invoice number (handles prefixes like 26-152 vs 26-1522).
export function pickInvoiceForFilename(filename, invoiceNos) {
  const base = String(filename).replace(/\.[^.]+$/, "").trim();
  const nb = base.toLowerCase();
  const cb = nb.replace(/\s+/g, "");
  let best = null;
  for (const no of invoiceNos) {
    if (!no) continue;
    const nno = String(no).toLowerCase();
    const cno = nno.replace(/\s+/g, "");
    const hit = nb.startsWith(nno) || cb.startsWith(cno) || nb.includes(nno);
    if (hit && (!best || String(no).length > String(best).length)) best = no;
  }
  return best;
}
