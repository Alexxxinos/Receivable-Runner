...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const g of groups) {
      ...
      // gather any invoice files attached to this client's overdue invoices
      const attachments = [];
      for (const id of g.ids) {
        const rec = byId.get(id);
        if (!rec?.attachment_path) continue;
        const { data, error: dlErr } = await supabase.storage
          .from("invoices")
          .download(rec.attachment_path);
        if (dlErr || !data) continue;
        const buf = Buffer.from(await data.arrayBuffer());
        attachments.push({ filename: rec.attachment_name || "invoice.pdf", content: buf.toString("base64") });
      }
